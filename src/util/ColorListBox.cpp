...
//
{
	...
	COLORREF crBkgr = ::GetSysColor(COLOR_WINDOW);
	BOOL bReverse = FALSE;
	if( GetRValue(crBkgr) + GetGValue(crBkgr) + GetBValue(crBkgr) < 3 * 128 ) bReverse = TRUE;

	COLORREF crNorm = (COLORREF)lpDIS->itemData; // Color information is in item data.
	if( ! lpDIS->itemData )	crNorm = ::GetSysColor(COLOR_WINDOWTEXT);
	else if( bReverse ) crNorm = RGB(255-GetRValue(crNorm), 255-GetGValue(crNorm), 255-GetBValue(crNorm));

	COLORREF crHilite = RGB(255-GetRValue(crNorm), 255-GetGValue(crNorm), 255-GetBValue(crNorm));
	if( ! lpDIS->itemData ) crHilite = ::GetSysColor(COLOR_HIGHLIGHTTEXT);

	COLORREF crGray = ::GetSysColor(COLOR_GRAYTEXT);
	...
	//	pDC->FillRect(&lpDIS->rcItem, &brush);
		pDC->FillSolidRect(&lpDIS->rcItem, crBkgr);
	}	 	
	...
	if (lpDIS->itemState & ODS_SELECTED)
		...
		crText = pDC->SetTextColor(crGray);
	else
