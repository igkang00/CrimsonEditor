...
- [ ] **FTPS (FTP over TLS)** — the remote-file feature is plain FTP only, so credentials and contents cross the wire in the clear. Explicit FTPS (`AUTH TLS`) would ride on the socket client `CFtpClient` (WinInet's FTP cannot do TLS). The handshake is small; the work is the TLS transport — ideally SChannel to avoid a third-party dependency — and the data-channel TLS session resumption that FileZilla and others require. SFTP (over SSH) is the larger, more common secure alternative but needs an SSH library
- [ ] **Agentic-coding companion features** — where a fast native editor fits alongside terminal AI coding agents (Claude Code, aider, …): a window to watch, review, and steer their edits rather than an AI IDE. Candidates: an AI-CLI bridge built on the existing user-tools mechanism (send a selection to a CLI, replace it in place), seamless auto-reload plus a local diff of what the agent changed, and apply-a-patch. Design exploration in [docs/agentic-coding.md](docs/agentic-coding.md)
