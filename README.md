...
- [x] **VS 2026 이관 완료** — `.dsp`/`.dsw` → `.vcxproj`/`.sln`, v145 toolset, MFC Dynamic + MBCS. 옛 VC6 빌드 파일은 역사적 자료로 보존.
- [ ] **소스/헤더 파일 디렉토리 정리** — 현재 약 180개 파일이 루트에 평평하게 있음. App/Doc/View/Frame/Panels/Dialogs/Network/Util/Core 등으로 분리 예정.
- [ ] **Unicode 빌드 검토** — 현재 MBCS만 지원. 한국어 입력기/IME 처리(`cedtViewEditCompose.cpp`)와 코드 전반의 `char`/`TCHAR` 가정 검토 필요.
...
- 저장소에 동봉된 [htmlhelp.lib](HtmlHelp.lib)는 옛 SDK 라이브러리로 `/SAFESEH`를 지원하지 않는다. Release 구성은 `ImageHasSafeExceptionHandlers=false`로 설정되어 있다.
- VC6 → 현대 toolset 이관 과정에서 한 호환성 처리는 다음과 같다:
    - `<fstream.h>` / `<strstrea.h>` → [fstream_compat.h](fstream_compat.h) shim으로 흡수 (글로벌 네임스페이스의 `ofstream` 등 using-선언)
