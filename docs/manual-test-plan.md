...
- [x] Print `Ctrl+P` `[cjk]` `[wrap]` · Print Preview (→ A2) · Print Setup. Fine (preview covered
      in A2, finding 8).
- [x] FTP: Open Remote `Ctrl+Shift+O` · FTP Settings — tested against a local FileZilla Server.
      The default socket backend was unusable (couldn't even connect) and needed a Unicode-migration
      fix; download/large-file/dialog issues surfaced and were fixed too (findings 37–39). Connect,
      list (incl. Korean names), download, upload, and the settings dialog all work now.
      **Skipped — no server.**
...
| 36 | §B File · **Save** (`Ctrl+S`) on an unmodified, already-saved file | **BUG (needless disk write + spurious prompt).** `FileSave` had no modified-state check, so Save on a file with no unsaved changes still rewrote it to disk — bumping its timestamp for nothing — and, after finding 35, popped the read-only "save under a new name?" prompt on a read-only file the user had merely opened and never edited. `OnFileSaveAll` already guarded with `IsModified()`; `FileSave` (Ctrl+S / File > Save) did not, and `ID_FILE_SAVE` has no update handler so the command is always enabled. | 3.94 Release-KR | **fixed** — `FileSave` returns early (`return TRUE`, no-op) when `! IsModified()`, placed **after** the new-unsaved-file check so a brand-new document still goes to Save As ([cedtDocFile.cpp](../src/doc/cedtDocFile.cpp)). Matches VS Code / Visual Studio / Sublime — Save on a clean file writes nothing and prompts nothing. For 3.94. |
| 37 | §B File · FTP · connect and list a directory with the **default (socket) backend** | **BUG (Unicode-migration regression — the default FTP backend was unusable).** `CFtpClient` runs the control channel as a `CArchive` over a `CSocketFile` and moved commands/replies with `WriteString`/`ReadString`, which under `_UNICODE` transfer 2-byte wide chars. FTP is a byte protocol, so commands went out as UTF-16 (unparseable) and replies were read back as wide chars — mojibake, and a response code that no longer began with three digits, so **every connection reported failure** (the garbled "could not connect" box). The socket client is the default (`m_bUseWinInet` defaults FALSE), so FTP was broken out of the box. | 3.94 Release-KR | **fixed** — move the control channel and the LIST data channel over the wire as **UTF-8 bytes**: commands via `WideCharToMultiByte(CP_UTF8)`, replies/listing read a byte at a time to the newline and decoded with `MultiByteToWideChar(CP_UTF8)` ([FtpClnt.cpp](../src/network/FtpClnt.cpp)). Connection, response codes, and **Korean file/dir names** (the server runs UTF8 mode) now work. Binary file transfer already used CArchive's byte-level `Read`/`Write`, so it was left as-is. WinInet backend kept as an option but it decodes names in the system code page (garbles non-ASCII) — the socket client is the correct path. For 3.94. |
| 38 | §B File · FTP · **download** a file (especially large) from FileZilla | **BUG (two download-path defects, exposed once finding 37 let connections through).** (1) The download aborted because it read the file size from the RETR reply expecting wu-ftpd's `150 … (NNNN bytes)`; FileZilla just says `150 Starting data transfer.`, so the parse failed and the whole transfer was treated as failed. (2) The progress bar filled, reset to 0, and filled again on files over ~43 MB — `100U * nTotalRead / dwSize` overflowed 32-bit `UINT` (100 × bytes wraps once `nTotalRead` ≈ 42.9 MB). | 3.94 Release-KR | **fixed** — (1) get the size up front with the `SIZE` command (RFC 3659; re-enabled the existing `GetFileSize`, switching to binary first since some servers refuse SIZE in ASCII) and treat a missing size as non-fatal — download without a percentage instead of aborting ([RemoteFile.cpp](../src/network/RemoteFile.cpp)). (2) Compute progress in 64-bit — `(UINT)((ULONGLONG)nTotalRead * 100 / dwSize)` — at all four transfer sites (download/upload × socket/WinInet). Verified with a 57 MB file: one clean 0→100 sweep. For 3.94. |
| 39 | §B File · FTP · **FTP Settings** dialog — Korean labels cramped | **Layout.** In `IDD_FTP_SETTINGS` the Korean labels sat tight and the "비밀번호 저장" (Save password) checkbox was clipped (61–64 dialog units wide). | 3.94 Release-KR | **fixed** — widened the dialog 243 → 273 units in **both** editions, extending the account list, the edit fields, and the right-hand button column, and the Save-password checkbox (→ 91–94 units), keeping the 7-unit right margin symmetric ([cedt_us.rc](../cedt_us.rc), [cedt_kr.rc](../cedt_kr.rc)). For 3.94. |
| 40 | §B Tools / View · saving user **tools / macros / colour schemes** failed silently under the read-only install dir | **BUG (silent failure + wrong default location).** The Save dialogs for User Tools, User Macros, and Colour Schemes defaulted to the **install directory** (`<install>\tools`, `<install>\schemes`) — under `Program Files`, not writable by a standard/UAC user — and the three save handlers **ignored the return value**, so a permission-denied save just did nothing. | 3.94 Release-KR | **fixed** — (1) the handlers now check the save result and report `IDS_ERR_SAVE_USER_FILE` ("Could not save to '%s' … choose a different location") on failure ([PrefDialogCommands.cpp](../src/dialogs/preferences/PrefDialogCommands.cpp), [PrefDialogMacros.cpp](../src/dialogs/preferences/PrefDialogMacros.cpp), [PrefDialogColors.cpp](../src/dialogs/preferences/PrefDialogColors.cpp)). (2) The default save/load location moved to the writable AppData tree — `%APPDATA%\Crimson Editor\{tools,macros,colors}\`, created at startup ([cedtapp.cpp](../src/app/cedtapp.cpp)); the Load-User-Tools / Load-User-Macros menus now scan those user folders ([cedtViewCommand.cpp](../src/view/cedtViewCommand.cpp), [cedtViewMacro.cpp](../src/view/cedtViewMacro.cpp)), and colour-scheme Load/Save (Preferences and View › Color Scheme › Saved) default there too ([cedtAppHndr.cpp](../src/app/cedtAppHndr.cpp)). No factory tool/macro/scheme files are bundled, so scanning only the user dir hides nothing; the startup fallback that reads a bundled default `cedt.colors` from the install dir is left as-is (read-only). For 3.94. |
