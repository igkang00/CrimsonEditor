...
14. **Status-bar help strings carried non-ASCII punctuation** — the v3.90 English `.rc` STRINGTABLE for the Encoding menu used an em dash (U+2014) and a multiplication sign (`~2×`, U+00D7). The `.rc` source is UTF-8, but those bytes were interpreted through the ANSI code page somewhere in the `.rc` → status-bar flyby-help pipeline, so `×` (UTF-8 `C3 97`) rendered as `Â—` (CP1252). Replaced both with ASCII (`-`, `x`) so the help text is code-page-independent. The Korean `.rc` was already clean (Hangul "배" for the multiplier, ASCII hyphens for dashes).
15. **BOM-less CP949 misdetected as UTF-8** — `DetectEncodingType` classified any BOM-less file that wasn't valid UTF-8 as `ENCODING_TYPE_UNKNOWN`, and the caller (`cedtDoc.cpp`) replaced UNKNOWN with the default encoding, which Phase 4i had flipped to UTF-8 w/o BOM. So a plain CP949 source file (`한글` = `c7 d1 …`, invalid UTF-8) loaded through `MultiByteToWideChar(CP_UTF8, …)` and every Korean byte became U+FFFD. Fix: when a byte above 0x7F fails UTF-8 validation, classify the file as `ENCODING_TYPE_ASCII` (legacy MBCS/ANSI), not UNKNOWN — only files with no non-ASCII byte at all stay UNKNOWN and reach the UTF-8 default.
16. **Registry REG_SZ read truncated to one character under `_UNICODE`** — `GetRegKeyValue(…, CString&)` read into a `BYTE szBuf[MAX_PATH]` with `dwSize = MAX_PATH`, then did `szValue = szBuf`. `RegQueryValueExW` writes UTF-16, but assigning a `BYTE*` to a `CStringW` reinterprets those bytes as ANSI, so `"C:\Program Files\Crimson Editor"` (`43 00 3A 00 …`) stopped at the first NUL and became `"C"`. Every install-dir-relative lookup (syntax-spec link files, color scheme, templates, schemes) then failed on the installed build — most visibly `.c` files got no syntax type. Fix: read into a `TCHAR` buffer sized in bytes and null-terminate; also scaled `SetRegKeyValue`'s `cbData` by `sizeof(TCHAR)` so writes aren't half-length.
