...
| 7a — Installer rebuild for 3.90 | 2026-07-06 17:31 | `dist\cedt-390-setup.exe` (25.85 MB) rebuilt end-to-end via `scripts\build_installer.ps1` after every Phase 4 / 5 fix landed; supersedes the earlier same-named artifact from Phase 3.8. **NOTE:** superseded again by 6c — the D2Coding bundle changes `installer.iss`, so this artifact must be rebuilt (with `runtime\fonts\` populated) before ship |
